...
def thread(args):
    setup, file, gamma, learning_rate, epochs, eps = args
    print(f"Start thread: gamme={gamma}, epoch={epoch}")
    q = QLearning(setup, gamma=gamma, learning_rate = learning_rate, epochs=epochs, eps = eps, name=DIRECTORY_PATH + file)
        ...
        for setup, file in zip(setups, files):
            for epoch in list_epochs:
                for l_r in learning_rates:
                    arguments = [(setup, file, 0.8, l_r, epoch, 0.9)]
